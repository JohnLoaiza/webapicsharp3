            ...
            Dictionary<string, object?> parametros)
        {
            ...
            for (int i = 0; i < parametrosEntrada.Count; i++)
            {
                ...
                else if (tipoMeta is "integer" or "int" or "int4")
                {
                    ...
                    comando.Parameters.Add(new NpgsqlParameter { Value = valor != DBNull.Value ? valorInt : DBNull.Value, NpgsqlDbType = NpgsqlDbType.Integer });
                }
                // Bigint
                else if (tipoMeta is "bigint" or "int8")
                {
                    long valorLong = valor == DBNull.Value ? 0 : Convert.ToInt64(valor);
                    comando.Parameters.Add(new NpgsqlParameter { Value = valor != DBNull.Value ? valorLong : DBNull.Value, NpgsqlDbType = NpgsqlDbType.Bigint });
                }
                // Numeric
                else if (tipoMeta is "numeric" or "decimal")
                {
                    decimal valorDec = valor == DBNull.Value ? 0 : Convert.ToDecimal(valor);
                    comando.Parameters.Add(new NpgsqlParameter { Value = valor != DBNull.Value ? valorDec : DBNull.Value, NpgsqlDbType = NpgsqlDbType.Numeric });
                }
                else
                {
            ...
            Console.WriteLine($"[SQL] {sqlLlamada}");
            foreach (NpgsqlParameter p in comando.Parameters)
            {
